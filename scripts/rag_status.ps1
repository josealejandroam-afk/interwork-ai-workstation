...
if (Test-Path $SQLITE_DB) {
    $dbSizeKB = [math]::Round((Get-Item $SQLITE_DB).Length / 1024, 1)
    Write-Host ("SQLite metadata:     Found (" + $dbSizeKB + " KB)") -ForegroundColor Green
} else {
...
if (Test-Path $RAG_DIR) {
    ...
    $output = & uv run python search.py "InterWork" 2>&1
    $hit = $output | Select-String -Pattern "score=" | Select-Object -First 1
    if ($hit) {
