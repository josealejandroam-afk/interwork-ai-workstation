...
from .runtime_state import finalize_recovery, inspect_lock, recover_stale_lock
...
def _maintenance(argv: list[str]) -> int | None:
    if not argv or argv[0] not in {"inspect-lock", "recover-lock", "recover-finalization"}:
        return None
    command = argv[0]
