# Open Loops — Project 7713

Updated September 14, 2026 by Codex@FrankWork. All remain OPEN. OW Houston-local project assignment is resolved as 7713; execution readiness is not. Existing matching database loops are updated in place, not duplicated.

| # | Item | Status |
|---|---|---|
| 1 | Confirm QuoteWerks quote for #7713 covers both Dallas relocation and OW Houston-local scope. | Open |
| 2 | Confirm vendor assignment. | Open |
| 3 | Obtain final marked inventory/deck, including quantities and handling of OW lockers/refrigerators from 500 Dallas. Lockers move unless OW cancels. | Open |
| 4 | Confirm final Aeron count. | Open |
| 5 | Confirm exact conference furniture quantities. | Open |
| 6 | Confirm Dallas-to-Houston transportation provider. | Open |
| 7 | Confirm truck/trailer requirements. | Open |
| 8 | Confirm whether loading begins October 9. | Open |
| 9 | Confirm Houston unloading responsibility. | Open |
| 10 | Confirm final Houston furniture installation responsibility. | Open |
| 11 | Confirm power pole quantity and locations. | Open |
| 12 | Confirm glassboard installation responsibility. | Open |
| 13 | Confirm final desktop technology scope. | Open |
| 14 | Confirm whether InterWork has any network/AP responsibility. | Open |
| 15 | Confirm Dallas onsite POC. | Open |
| 16 | Confirm Houston onsite POC. | Open |
| 17 | Confirm access requirements at 1717 Main, 500 Dallas Street and 1100 Louisiana. | Open |
| 18 | Obtain 1717 security/clearance form if required. | Open |
| 19 | Confirm Houston site-ready date. | Open |
| 20 | Resolve OW packing end-date discrepancy. | Open |
| 21 | Confirm whether separate Dallas and Houston FastFields are required. | Open |
| 22 | Confirm whether InterWork will purchase the new ping pong table. | Open |
| 23 | Confirm exact disposition of excess Aerons at Houston. | Open |
| 24 | Confirm conference furniture shipping sequence between Dallas and Houston. | Open |
| 25 | Confirm October 19 OW supplies delivery window, resources and access at 500 Dallas: 25 crates + 4 C-bins. | Open |
| 26 | Confirm exact OW Houston-local move dates during week of November 9, crew/trucks and dock/freight reservations at both Houston sites. | Open |

## Earlier unresolved controls retained

- AV responsibility remains unassigned.
- Electrical execution beyond confirmed power pole provision and coordination remains unassigned.
- Confirm separate project/billing assignment for Veritas at 1200 Smith, its decommission and CORT rental pickup; do not include under 7713. OW 500 Dallas activity IS 7713. The identified Rancho conference table is assigned to 7713; remaining table execution/commercial controls are tracked separately below.

## HIS conference table controls — September 14

- [ ] Provide scope to HIS and obtain quote/job for October work or table-only receiving/delivery/install; not Osha Bergman quote 59829.
- [ ] Confirm HIS receipt date, 30-day storage clock/rate, paperwork and final delivery/install instructions; hold until directed. Coordinate Kathy's September 18–28 PTO coverage if needed.
