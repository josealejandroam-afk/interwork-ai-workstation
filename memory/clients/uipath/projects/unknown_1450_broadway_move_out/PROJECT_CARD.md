# Project TBD — UiPath 1450 Broadway Move Out (NYC)

## Core Facts

| Field | Value |
|---|---|
| Project number | **Unknown — needs assignment** |
| Client | UiPath |
| Project name | 1450 Broadway Move Out |
| Location | 1450 Broadway, 21st Floor, New York, NY 10018 |
| Scope | Relocate ~55 smaller boxes; majority to donation locations (labeled/grouped); a few to 1 Vanderbilt; dispose of additional items |
| Initial request | 2026-06-12 |
| **New execution date** | **Wednesday, July 8, 2026, 11:00 AM start** |
| Status | **Rescheduled — date confirmed by client** |
| Source of confirmation | Morgan Alvarado email, 2026-06-30 2:01 PM |

## Contacts

| Role | Name | Phone |
|---|---|---|
| Client — Workplace Experience Manager | Morgan Alvarado | 929-996-5850 |
| Client — Onsite contact | Michele Nerio | 512-426-1492 |
| Client | Simon Chey | — |
| InterWork PM | David Steinbrecher | — |
| InterWork | Francisco Vinueza | — |
| InterWork | InterWork Operations | — |

## Freight / Building Access

- **Freight entry:** 41st Street between 6th Ave and Broadway, door next to FedEx
- **COI:** Building management has the InterWork COI on file
- **Floor:** 21st Floor

## Scope Detail

- **~55 smaller boxes** to be relocated
- **1 Vanderbilt:** a few boxes go here
- **Donation locations (majority, labeled/grouped):**

| Destination | Address | Hours | Notes |
|---|---|---|---|
| Materials for the Arts | 33-00 Northern Blvd, LIC 11101 | M/W/F, 9am–2:30pm | Dock available; arrive before 2:15 PM |
| Goodwill NYNJ Donation Center | 1114 1st Ave, NY 10065 | M–Sun, 11am–7pm | — |
| Housing Works | 157 E 23rd St, NY 10010 | M–Sat, 12pm–7pm | — |

- **Additional items:** dispose of (beyond the ~55 boxes)

## Source Notes

- Execution date, address, floor, scope, freight access, and donation locations confirmed via Morgan Alvarado email 2026-06-30 2:01 PM.
- Project number still unassigned — do not invent one; folder slug remains `unknown_1450_broadway_move_out`.
- Alejandro's prior email to Michele (sent 2026-06-30 12:51 PM) referenced the now-superseded July 1 date — it is superseded by Morgan's reschedule email.
- Confidence: High for confirmed facts above; Medium for anything not listed here.
- Created: 2026-06-30 | Last updated: 2026-06-30
