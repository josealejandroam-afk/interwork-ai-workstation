# Project 7700 — Radian Group Bethesda Move and Decommission

## Project Identity

| Field | Value |
|---|---|
| Project number | 7700 |
| Quote | 8677, dated 2026-09-08 |
| Client | Radian Group Inc. |
| Site | 7315 Wisconsin Avenue, Bethesda, MD 20814 |
| Client POC | Kristopher “Kris” Blalock, 856-905-9802, Kristopher.Blalock@radian.com |
| InterWork PM | Melvin Hernandez |
| Project type | Move and decommission |
| Status | Planning / scope development |

## Current Evidence

- New-project announcement identifies 7700 as the Radian Bethesda move and decommission.
- Walkthrough occurred as scheduled: **9/8/2026, 9:00 AM** — Kris Blalock (Radian FM) on
  site with InterWork field PM Melvin Hernandez to box up an office and run a site survey
  for subtenant decluttering. Full findings from the rest of the walkthrough are not yet
  documented beyond the FedEx errand below.
- During that visit, Kris Blalock gave Melvin Hernandez ~8 boxes to take to FedEx on the
  client's behalf. Melvin completed this — confirmed by Alejandro via phone call with
  Melvin, 2026-09-08. This is a client-requested favor, not part of the formal scope.
- Packing assistance was discussed for September 14-15, 2026 but is not confirmed as
  awarded execution. Jill Buchman confirmed 9/8 and 9/14-9/15 are on InterWork's internal
  schedule (per 9/2 email) — this is InterWork-side calendar holding, not client
  confirmation of loading-dock access or start time.
- Quote 8677 was created September 8, 2026 for Project 7700.
- The visible proposal has a blank scope and a $0 total; it is a placeholder/commercial shell, not a priced move authorization.
- **COI: RESOLVED.** Scott Rasmussen sent the COI back to Kris Blalock 9/8, 11:52 AM.
  Certificate Holder: Bethesda Crossing EW Acquisition LLC, c/o MRP Real Estate Services
  Group, 7315 Wisconsin Ave, Suite 420E, Bethesda, MD 20814. Description of Operations
  lists Bethesda Crossing EW Acquisition LLC and MRP Real Estate Services Group LLC as
  additional insured. The earlier "EW Towers" vs. "Bethesda Crossing" naming difference is
  resolved — "EW Tower(s)" is the building/property reference, "Bethesda Crossing EW
  Acquisition LLC" is the legal entity used on the COI; they're not conflicting
  destinations. Confirmed via the issued certificate (filed as "COI - Bethesda
  Crossing.pdf") and Scott's original request, which referenced "EW Tower."
- **Scope classification open:** Alejandro's working label for this project is "packing
  help and move to storage." The live Supabase record currently types it as
  `decommission`. These need reconciling — do not assume either is correct without
  confirming actual scope.

## Contacts

| Name | Role | Contact |
|---|---|---|
| Kristopher "Kris" Blalock, FMP | Facilities Manager, Radian Group Inc — primary client contact | Kristopher.Blalock@radian.com, 856-905-9802 (already on file via 7492) |
| Sharon Jones | Facilities Manager, Radian Group — cc'd | Sharon.Jones@radian.com (already on file via 7492) |
| Jill Buchman | InterWork Account Manager | jillb@interworkoffice.com, 609-744-8911 |
| Scott Rasmussen | InterWork Operations Project Manager — sent the COI | ScottR@interworkoffice.com, 856-816-0532 |
| Francisco Vinueza | InterWork | — |
| David Steinbrecher | InterWork | Phone unconfirmed |
| Melvin Hernandez | InterWork field PM — confirmed execution owner in Supabase | — |

## Scope Boundary

Do not treat the walkthrough findings (beyond the FedEx errand), packing dates,
loading-dock week, 9/14-9/15 start time/POC, final move/decommission scope, price,
execution schedule, or scope classification (decommission vs. packing/move to storage)
as confirmed until documented and reconciled. (COI and the EW Towers/Bethesda Crossing
naming question are resolved — see Current Evidence above.)
