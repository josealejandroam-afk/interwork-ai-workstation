# Open Loops — Project 7492 Radian Denver Decom

| # | Item | Status | Source |
|---|---|---|---|
| 1 | Identify exact items inadvertently removed by Barraza | Needs confirmation | Prior sessions |
| 2 | Confirm inadvertently removed items NOT in return shipment | CONFIRMED — Mike returning separately to Denver | Alejandro 2026-07-01 |
| 3 | Confirm Mike completed return of inadvertent items to Denver client | Needs confirmation | Prior sessions / Alejandro 2026-07-01 |
| 4 | Confirm final Wayne, PA street address for copier delivery | Open — still TBD | Alejandro 2026-07-01 |
| 5 | Confirm Mike Barraza phone number from safe internal source | Needs confirmation | Prior sessions |
| 6 | Confirm final WC report status | Needs confirmation | Prior sessions |
| 7 | Confirm final invoice status | Needs confirmation | Prior sessions |
| 8 | Confirm June 22 return visit completed (security/IDF items, blank plates) | CONFIRMED COMPLETE | Alejandro 2026-07-01 |
| 9 | Confirm Cherry Hill delivery completed (111 Woodcrest Rd, 2nd floor) | Not yet — scheduled July 6, 9:30 AM | Alejandro 2026-07-01 |
| 10 | Confirm Wayne, PA copier delivery completed | Not yet — follows Cherry Hill on July 6 | Alejandro 2026-07-01 |
| 11 | Confirm Sunset pickup / return shipment arrived NJ warehouse | CONFIRMED COMPLETE | Alejandro 2026-07-01 |
| 12 | COI — obtain and send to Wayne building property management before July 6 | Open — new item; owner TBD | Email thread 2026-07-01 |
| 13 | Sharon Jones cell number — needed for field handoff/delivery-team contact before July 6 | CONFIRMED — 720-486-8789 (Mobile) | Alejandro 2026-07-01 |
| 14 | Client PM notification — confirm Pedro Martinez contact info was sent to client | Open — Francisco committed EOD June 26, unconfirmed sent as of July 1 | Email thread 2026-07-01 |

## NOTE: Two Separate Logistics Issues

Items inadvertently removed (loops 1 and 3) and the Sunset pickup/return shipment (loop 11) are UNRELATED. Loop 11 is confirmed closed. Loops 1 and 3 remain open. Track and communicate them separately.

## Loops Confirmed Closed This Session (2026-07-01)

- Loop 2: Inadvertent items NOT in Cherry Hill/return shipment — confirmed
- Loop 8: June 22 return visit — confirmed complete
- Loop 11: Sunset pickup / return shipment to NJ warehouse — confirmed complete
- Loop 13: Sharon Jones cell number — confirmed (720-486-8789), provided directly by Alejandro
