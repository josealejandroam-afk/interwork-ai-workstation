# Project 7492 — Radian Denver, CO Decommission and Move
...
## Scope: 26th Floor Disposal

- (15) whiteboards
- (2) small screens
- (1) Land Acknowledgment picture
- Additional furniture (quantity Needs confirmation)

## Scope: Security Removal
...
- Trace and pull cabling to nearest IDF or MDF
- Risers excluded unless explicitly included

## Scope: Outbound Staging (for client receiving)
...
There are TWO separate unrelated issues that must NOT be merged in communications:

1. **Items inadvertently removed** — equipment removed during decom that was not intended for removal and needs to be returned to the client site
2. **Sunset pickup** — separate, planned logistics event

Keep client-facing messages about each issue separate.
Do not reference the vendor problem (inadvertent removal) in client communications unless necessary.

## Truck and Access Notes

- 26-foot truck relevant for logistics planning
- Confirm whether dock is available or liftgate is required when planning pickup or delivery
- Confirm exact address when sending vendor-facing logistics instructions
- E-waste COD noted — confirm disposal responsibility
